...
export class Orchestrator {
  ...
  async rewrite(evt: ChatEvent, text: string): Promise<string> {
    ...
    const sys =
      evt.chatType === "group"
        ? `你是“回复润色器”。把要发给用户的文本润色成更像真人在群里聊天的口吻。\n要求：\n- 保持事实不变：不要改数字、时间、ID、链接、列表顺序。\n- 简短但信息完整：不要为了短而删掉关键点。\n- 不要自称机器人/AI/助手，不要说“作为AI”。\n- 不要输出 Markdown，不要输出 JSON，不要输出代码块。\n- 不要输出“我来为你总结/以下是”等模板话。`
        : `你是“回复润色器”。把要发给用户的文本润色成更像真人私聊的口吻。\n要求：\n- 保持事实不变：不要改数字、时间、ID、链接、列表顺序。\n- 自然直接但信息完整，避免官腔和模板化。\n- 不要自称机器人/AI/助手，不要说“作为AI”。\n- 不要输出 Markdown，不要输出 JSON，不要输出代码块。`;
  ...
  private formatOutput(evt: ChatEvent, text: string): string {
    ...
    if (evt.chatType === "group") {
      return limitChatText(cleaned, { maxChars: 1800, maxLines: 18 });
    }
    return limitChatText(cleaned, { maxChars: 3800, maxLines: 28 });
  }
  ...
  async handle(
    ...
    const systemParts: string[] = [
      `你的昵称是${this.config.BOT_NAME}。`,
      "像真人一样自然聊天，语气口语化，避免官腔和模板化。",
      "不要主动提自己是机器人/AI/助手，不要说“作为AI”。",
      "默认短回复：先给结论，再给 1-3 个关键信息点或下一步。",
      "信息不足不要硬编：只问 1 个最关键的问题，把问题问得具体。",
      evt.chatType === "group"
